...
def test_image_overview_wrap():
    ...
    assert np.array_equal(
        fig.get_size_inches(),
        np.array([9, 6]),  # 3 columns * 3 width, 2 rows * 3 height
    )
...
def test_image_overview_nowrap():
    ...
    assert np.array_equal(
        fig.get_size_inches(),
        np.array([9, 3]),  # 3 columns * 3 width, 2 rows * 3 height
    )
...
def image_and_label_sets_dict():
    ...
    label_set = {
        ...
        'colormap': [
            None,
            'Labels',
        ],  # tests the cmap.lower() component, this actually helped because it detected a lack of list
        'title': [None, 'Labels'],
    }
...
def test_image_overview_scalebar_float(image_and_label_sets_ImageSet):
    ...
    for ax in fig.axes:
        scalebar = [
            child for child in ax.get_children() if isinstance(child, ScaleBar)
        ]
